...
/*            $Id: CpriRSA.c 472 2015-12-22 22:43:40Z kgoldman $		*/
/*										*/
...
// This function computes the private exponent de = 1 mod (p-1)*(q-1) The inputs are the public
// modulus and one of the primes.  The results are returned in the key->private structure. The size
// of that structure is expanded to hold the private exponent. If the computed value is smaller than
// the public modulus, the private exponent is de-normalized.

// Return Value	Meaning
// CRYPT_SUCCESS	private exponent computed

// CRYPT_PARAMETER prime is not half the size of the modulus, or the modulus is not evenly divisible
// by the prime, or no private exponent could be computed from the input parameters
...
// insufficient space is available, the size is not changed and the return code is CRYPT_FAIL.
	   ...
	   )
{
	...
	*pm++ ^= *pp++;
    ...
    // Make sure that the end of the pad was found and it was 0x01
    if(i == 0 || pm[-1] != 0x01)
	return CRYPT_PARAMETER;
...
// B.12.1.3.10.	PKSC1v1_5SignEncode()
...
// Function that is called to initialize the hash service. In this implementation, this function
// does nothing but it is called by the CryptUtilStartup() function and must be present.
...
// B.12.1.4.2.	_cpri__TestKeyRSA()

/* This function computes the private exponent de = 1 mod (p-1)*(q-1) The inputs are the public
   modulus and one of the primes or two primes. */

/*     If both primes are provided, the public modulus is computed. If only one prime is provided,
       the second prime is computed. In either case, a private exponent is produced and placed in
       d. */
...
// CRYPT_PARAMETER cInSize is not the same as the size of the public modulus of key; or numeric
// value of the encrypted data is greater than the modulus
...
// B.12.1.4.6.	_cpri__ValidateSignatureRSA()

// This function is used to validate an RSA signature. If the signature is valid CRYPT_SUCCESS is
// returned. If the signature is not valid, CRYPT_FAIL is returned. Other return codes indicate
			    ...
			    )
{
    ...
    // make sure that scheme is supported
    switch (scheme)
	{
	  case TPM_ALG_RSAPSS:
	  case TPM_ALG_RSASSA:
    ...
    switch (scheme)
	{
	  ...
	  default:
	    FAIL(FATAL_ERROR_INTERNAL);
	    return CRYPT_SCHEME;	/* kgold: added to silence gcc */
	}
}

#ifndef RSA_KEY_SIEVE
// B.12.1.4.7.	_cpri__GenerateKeyRSA()
// Generate an RSA key from a provided seed
// Return Value	Meaning

// CRYPT_FAIL exponent is not prime or is less than 3; or could not find a prime using the provided
// parameters
...
_cpri__GenerateKeyRSA(
		      ...
		      //     to be propagated across multiple routines
		      )
{

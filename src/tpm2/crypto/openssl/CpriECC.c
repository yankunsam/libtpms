...
/*            $Id: CpriECC.c 477 2015-12-23 18:43:05Z kgoldman $		*/
/*										*/
...
// In this implementation, no initialization is performed at startup but a future version may
// initialize the self-test functions here.
...
// B.13.3.2.3.	_cpri__EccGetCurveCount

// This function returns the count of curves supported by this implementation.
...
// B.13.3.2.4.	_cpri__EccGetParametersByCurveId()

// This function returns a pointer to the curve data that is associated with the indicated
// curveId. If there is no curve with the indicated ID, the function fails.
...
// B.13.3.2.5.	GetCurveData()

// Function to get the address of the ECC_CURVE_DATA associated with a curveId. If the curve is not
// implemented, then this should not be called.
...
// B.13.3.2.10.	PointMul()

// This function does a point multiply and checks for the result being the point at infinity. Q =
// ([A]G + [B]P)
...
// B.13.3.2.11.	GetRandomPrivate()

// This function gets a random value (d) to use as a private ECC key and then qualifies the key so
// that it is between 0 < d < n.

// It is a fatal error if dOut or pIn is not provided or if the size of pIn is larger than
// MAX_ECC_KEY_BYTES (the largest buffer size of a TPM2B_ECC_PARAMETER)
...
// B.13.3.2.13. _cpri__EccPointMultiply
			...
			)
{
	...
	Point2B(group, Rout, R,
		(INT16)BITS_TO_BYTES(EC_GROUP_get_degree(group)),
		context);
...
// B.13.3.2.15.	_cpri__EccCommitCompute()
...
// It is a fatal error if r or d is NULL. If B is not NULL, then it is a fatal error if K and L are
// both NULL. If M is not NULL, then it is a fatal error if E is NULL.
			...
			)
{
    ...
    keySizeInBytes = (INT16)BITS_TO_BYTES(EC_GROUP_get_degree(group));
...
// B.13.3.2.16.	_cpri__EccIsPointOnCurve()
...
// B.13.3.2.17.	_cpri__GenerateKeyEcc()

// This function generates an ECC key pair based on the input parameters. This routine uses KDFa()
// to produce candidate numbers. The method is according to FIPS 186-3, section B.4.1 "GKey() Pair
// Generation Using Extra Random Bits." According to the method in FIPS 186-3, the resulting private
// value d should be 1 <= d < n where n is the order of the base point. In this implementation, the
// range of the private value is further restricted to be 2^(nLen/2) <= d < n where nLen is the
// order of n.
		      ...
		      )
{
    ...
    pAssert(seed != NULL && dOut != NULL && Qout != NULL);
...
// B.13.3.2.18.	_cpri__GetEphemeralEcc()
...
// This function will modify the digest before it is used in Ecdsa sign or
// verify. It is localizes because the way that this is done could change
// on a whim. Currently, this is implemented as in the Windows 10 version of CNG
// and in OpenSSL. It is truly stupid that they ended up with this implementation.
// If they had just modular reduced the digest, there wouldn't be any issues with
// things like P521. But... 

static BOOL
EcdsaDigest(
	    BIGNUM                  *dOut,       // OUT: an adjusted value
	    TPM2B                   *dIn,        // IN: digest to 'fix'
	    UINT32                   orderBits   // IN: the number of bits in the order.
	    )
{
    int                 shift;
    BN_bin2bn(dIn->buffer, MIN(dIn->size, BITS_TO_BYTES(orderBits)), dOut);
    shift = BN_num_bits(dOut) - orderBits;
    if(shift > 0)
        BN_rshift(dOut, dOut, shift);
    return TRUE;
}

// B.13.3.2.19.	SignEcdsa()

// This function implements the ECDSA signing algorithm. The method is described in the comments
// below. It is a fatal error if rOut, sOut, dIn, or digest are not provided.
	  ...
	  )
{
    ...
    // Assume the size variables do not overflow, which should not happen in
    // the contexts that this function will be called.
    if(curveData->n->size > MAX_ECC_KEY_BYTES)
    ...
    for (;;) // This implements the loop at step 6. If s is zero, start over.
	{
	    for (;;)
		{
	    ...
	    EcdsaDigest(bnZ, digest, BN_num_bits(bnN));
...
// B.13.3.2.24.	ValidateSignatureEcdsa()
		       ...
		       )
{
    ...
    // The curve selector should have been filtered by the unmarshaling process. If
    // it wasn't GetCurveData() does filtering.
    n = curveData->n;
   ...
   // Get an adjusted value of the hash
    EcdsaDigest(bnE, digest, BN_num_bits(bnN));
...
// B.13.3.2.25.	ValidateSignatureEcSchnorr()

// This function is used to validate an EC Schnorr signature. rIn and sIn are required to be greater
// than zero. This is checked in _cpri__ValidateSignatureEcc().
			   ...
			   )
{
    ...
    // The curve parameter should have been filtered by unmarshaling code and
    // checked by GetCurveData.
...
// B.13.3.2.26.	ValidateSignatueSM2Dsa()
			...
			)
{
    ...
    bnT = BN_CTX_get(context);
    bnS = BN_CTX_get(context);
    bnOrder = BN_CTX_get(context);

    if(   bnOrder == NULL
	  || (group = EccCurveInit(curveId, context)) == NULL)
	FAIL(FATAL_ERROR_INTERNAL);
    
    EC_GROUP_get_order(group,  bnOrder, context);
    ...
    fail = (BN_ucmp(bnR, bnOrder) >= 0);
    
    fail = (BN_ucmp(bnS, bnOrder) >= 0) || fail;
    if(fail)
    ...
    if(!BN_mod_add(bnT, bnR, bnS, bnOrder, context))
	FAIL(FATAL_ERROR_INTERNAL);
#ifdef _SM2_SIGN_DEBUG
    pAssert(cmp_bn2hex(bnT,
	...
	// trouble. If the copy fails (returns NULL) then it is fatal.
	if(!BN_copy(bnT, bnS))
	    FAIL(FATAL_ERROR_INTERNAL);
    }
    ...
    if(!BN_mod_add(bnRp, bnE, bnT, bnOrder, context))
	FAIL(FATAL_ERROR_INTERNAL);
...
// B.13.3.2.27.	_cpri__ValidateSignatureEcc()
...
#endif
	  default:
	    // Since we filtered the scheme above, there is something fatally wrong
	    // if we get here.
	    FAIL(FATAL_ERROR_INTERNAL);
	}
    return retVal;
...
// B.13.3.2.31.	SM2KeyExchange()

// This function performs the key exchange defined in SM2. The first step is to compute tA = (dsA +
// deA avf(Xe,A)) mod n Then, compute the Z value from outZ = (h tA mod n) (QsA +
	   ...
	   )
{
    ...
    size = (INT16)BN_num_bytes(bnOrder);
...
// B.13.3.2.33.	_cpri__C_2_2_KeyExchange()
...
#else       //%
used when the 2-phase key exchange is not defined so that the linker has something to associate with the value in the .def file.
LIB_EXPORT CRYPT_RESULT
_cpri__C_2_2_KeyExchange(
			 void
			 )

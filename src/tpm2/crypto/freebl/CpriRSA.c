...
static CRYPT_RESULT
RsaPrivateExponent(
    RSA_KEY         *key        // IN: the key to augment with the private exponent
)
{
    mpz_t            bnD;
    mpz_t            bnN;
    ...
    mpz_bin2mpz(key->publicKey->buffer, key->publicKey->size, bnN);
    mpz_bin2mpz(key->privateKey->buffer, key->privateKey->size, bnP);
    ...
    if(mpz_num_bits(bnP) < mpz_num_bits(bnN)/2)
    {
        retVal = CRYPT_PARAMETER;
        goto Cleanup;
    }
    // Get q = n/p;
    mpz_fdiv_qr(bnQ, bnQr, bnN, bnP);

    // If there is a remainder, then this is not a valid n
    if(mpz_num_bis(bnQr) != 0 || mpz_num_bits(bnQ) != mpz_num_bits(bnP))
    {
    ...
    mpz_set(bnPhi, bnN);
    mpz_sub(bnPhi, bnPhi, bnP);
    mpz_sub(bnPhi, bnPhi, bnQ);
    mpz_add_ui(bnPhi, bnPhi, 1UL);

    // Compute the multiplicative inverse
    mpz_set_ui(bnE, key->exponent);
    mpz_invert(bnD, bnE, bnPhi);
    //{
    ...
    fill = key->publicKey->size - mpz_num_bytes(bnD);
    mpz_mpz2bin(bnD, &key->privateKey->buffer[fill]);
...
)
{
    ...
    mpz_init(bnE);      // public exponent (e)
    mpz_init(bnD);      // private exponent (d)
    mpz_init(bnN);      // public modulus (n)
    mpz_init(bnP);
    mpz_init(bnPhi);
    mpz_init(bnQ);
    mpz_init(bnQr);

    mpz_bin2mpz(prime1->buffer, prime1->size, bnP);

    // If prime2 is provided, then compute n
    if(prime2 != NULL)
    {
        // Two primes provided so use them to compute n
        mpz_bin2mpz(prime2->buffer, prime2->size, bnQ);

        // Make sure that the sizes of the primes are compatible
        if(mpz_num_bits(bnQ) != mpz_num_bits(bnP))
        {
            retVal = CRYPT_PARAMETER;
            goto Cleanup;
        }
        ...
        if(mpz_num_bits(bnN) != (publicKey->size * 8))
        {
            retVal = CRYPT_PARAMETER;
            goto Cleanup;
        }
        mpz_mpz2bin(bnN, publicKey->buffer);
    }
    else
    {
        // One prime provided so find the second prime by division
        mpz_bin2mpz(publicKey->buffer, publicKey->size, bnN);

        // Get q = n/p;
        mpz_fdiv_qr(bnQ, bnQr, bnN, bnP);

        // If there is a remainder, then this is not a valid n
        if(mpz_num_bits(bnQr) != 0 || mpz_num_bits(bnQ) != mpz_num_bits(bnP))
        {
            retVal = CRYPT_PARAMETER;      // problem may be recoverable
            goto Cleanup;
        }
    ...
    mpz_sub(bnPhi, bnPhi, bnP);
    mpz_sub(bnPhi, bnPhi, bnQ);
    mpz_add_ui(bnPhi, bnPhi, 1);
    // Compute the multiplicative inverse
    mpz_set_ui(bnE, exponent);
    mpz_invert(bnD, bnE, bnPhi);
    //{
    ...
    fill = d->size - mpz_num_bytes(bnD);
    mpz_mpz2bin(bnD, &d->buffer[fill]);
...
)
{
    ...
    for(outer += 1; outer != 0; outer++)
    {
        ...
        if (mpz_cmp_ui(bnQ, 0) != 0)
        {
            ...
            if (mpz_cmpabs(bnP, bnQ) < 0)
                mpz_sub(bnT, bnQ, bnP);
            else
                mpz_sub(bnT, bnP, bnQ);
            if(mpz_num_bits(bnT) < 100) { // Difference has to be at least 100 bits
                mpz_clear(bnT);
                continue;
            }
        }
        ...
        MpzTo2B(n, bnN, 0);  // Fills the buffer with the correct size
        ...
        // Do a trial encryption and decryption of the seed to see if this
        // gives a valid result
        mpz_bin2mpz(seed->buffer, (n->size)-1, bnP);
        mpz_set(bnQ, bnP);
        mpz_powm(bnQ, bnQ, bnE, bnN);
        mpz_powm(bnQ, bnQ, bnT, bnN);
        if(mpz_cmp(bnP, bnQ) != 0) // Trial encrypt decrypt failed. Start
                                  // over with new primes
        {
            mpz_set_ui(bnQ, 0);
            continue;
        }

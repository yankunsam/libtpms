		     ...
		     )
{
    ...
    int          i;
    BYTE         ivec[16] = {0, };
    unsigned int keySize = (keySizeInBits + 7) / 8;
    ...
    if (!(AesKey = AES_CreateContext(key, ivec, NSS_AES, 1, keySize, keySize)))

    // there seems to be a bug in gcc (when using -O2) that the
    // loop below is not run unless we touch dSize here
    dSize = (dSize << 1) >> 1;
    ...
    for(; dSize > 0; dSize -= 16)
    {
        ...
        for(i = 16; i > 0; i--)
            *pIv++ ^= *dIn++;
        if (AES_Encrypt(AesKey, iv, &OutputLen, 16, iv, 16) != SECSuccess)
            FAIL(FATAL_ERROR_INTERNAL);
        pIv = iv;
        for(i = 16; i > 0; i--)
            *dOut++ = *pIv++;
    }
    AES_DestroyContext(AesKey, PR_TRUE);
    return CRYPT_SUCCESS;
}
		     ...
		     )
{
    ...
    if (!(AesKey = AES_CreateContext(key, ivec, NSS_AES, 0, keySize, keySize)))
        FAIL(FATAL_ERROR_INTERNAL);
    ...
    for(; dSize > 0; dSize -= 16)
    {
        ...
        for(i = 16; i> 0; i--)
            *pT++ = *dIn++;
        if (AES_Decrypt(AesKey, dOut, &OutputLen, 16, tmp, 16) != SECSuccess)
            FAIL(FATAL_ERROR_INTERNAL);
        pIv = iv;
        pT = tmp;
        for(i = 16; i> 0; i--)
		     ...
		     )
{
    ...
    for(; dSize > 0; dSize -= 16)
    {
        ...
        if (AES_Encrypt(AesKey, tmp, &OutputLen, sizeof(tmp),
            iv, sizeof(tmp)) != SECSuccess)
            FAIL(FATAL_ERROR_INTERNAL);
        pT = tmp;
        pIv = iv;
		     ...
		     )
{
    ...
    for(; dSize > 0; dSize -= 16)
    {
        ...
        if (AES_Encrypt(AesKey, dOut, &OutputLen, 16, dIn, 16) != SECSuccess)
            FAIL(FATAL_ERROR_INTERNAL);
        dIn = &dIn[16];
        dOut = &dOut[16];
    }
		     ...
		     )
{
    ...
    for(; dSize > 0; dSize -= 16)
    {
        ...
        if (AES_Decrypt(AesKey, dOut, &OutputLen, 16, dIn, 16) != SECSuccess)
            FAIL(FATAL_ERROR_INTERNAL);
        dIn = &dIn[16];
        dOut = &dOut[16];
    }

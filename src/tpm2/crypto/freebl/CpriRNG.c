...
/*                        Derived from openssl/CpriRNG.c			*/
...
_cpri__RngStartup(void)
{
    ...
    if(returnedSize > 0)
	{
	    // Seed FreeBL with entropy
	    RNG_SystemInfoForRNG();
	    _cpri__StirRandom(entropySize, entropy);
	}
    else
	{
	    s_entropyFailure = TRUE;
		      ...
		      )
{
    ...
    if(RNG_GenerateGlobalRandomBytes(buffer, randomSize) == SECSuccess)
		  ...
		  )
{
    if (entropySize >= 0)
	{
	    ...
	    RNG_RandomUpdate((const void *)entropy, (int)entropySize);
#else

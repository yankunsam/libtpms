...
/*			  Derived from openssl/RSAKeySieve.c			*/
...
// This next line will show up in the header file for this code. It will make the local functions
// public when debugging.
...
// These functions operate on a bit array. A bit array is an array of bytes with the 0th byte being
// the byte with the lowest memory address. Within the byte, bit 0 is the least significant bit.
...
void
ClearBit(
	 unsigned char   *a,             // IN: A pointer to an array of bytes
...
// This function finds the nth SET bit in a bit array. The caller should check that the offset of
// the returned value is not out of range.  If called when the array does not have n bits set, it
// will return a fatal error
...
// structure that contains pointers to a pre-computed set of hash contexts that are set up for the
// HMAC computations using the seed.

// This function will test that ktx.outer will not wrap to zero if incremented. If so, the function
// returns FALSE. Otherwise, the ktx.outer is incremented before each number is generated.
...
// number. I all likelihood, if the number is not prime, the first test fails.

// If a KDFa(), PRNG context is provide (ktx), then it is used to provide the random
// values. Otherwise, the random numbers are retrieved from the random number generator.
...
step4point1:
        ...
        mpz_bin2mpz(b.t.buffer, b.t.size, bnB);

// 4.2 If ((b ≤ 1) or (b ≥ w–1)), then go to step 4.1.
        if(mpz_cmp_ui(bnB, 0) == 0)
            goto step4point1;
        if(mpz_cmp_ui(bnB, 1) == 0)
            goto step4point1;
        if(mpz_cmpabs(bnB, bnWm1) >= 0)
            goto step4point1;

// 4.3 z = b^m mod w.
        mpz_powm(bnZ, bnB, bnM, bnW);

// 4.4 If ((z = 1) or (z = w − 1)), then go to step 4.7.
        if(mpz_cmp_ui(bnZ, 1) == 0 || mpz_abscmp(bnZ, bnWm1) == 0)
            goto step4point7;
...
PrimeSieve(
    mpz_ptr      bnN,       // IN/OUT: number to sieve
    UINT16       fieldSize, // IN: size of the field area in bytes
    BYTE        *field,     // IN: field
    UINT16       primes     // IN: the number of primes to use
    )
{
    UINT16           i;
    UINT32           j;
    UINT16           fieldBits = fieldSize * 8;
    UINT32           r;
    const BYTE      *p1;
    BYTE            *p2;
    PRIME_ITERATOR   iter;
    UINT32           adjust;
    UINT32           mark = 0;
    UINT16           count = sieveMarks[0].count;
    UINT16           stop = sieveMarks[0].prime;
    UINT32           composite;
...
//    UINT64           test;      //DEBUG
    ...
    while(composite = NextPrime(&iter))
    {
        UINT16  pList[8];
        UINT16   next = 0;
        i = count;
        ...
        composite = mpz_mod_ui(bnN, bnN, composite);
        for(i = count; i > 0; i--)
        {
            next = pList[i];
            if(next == 0)
...
PrimeSelectWithSieve(
    mpz_ptr          bnP,               // IN/OUT: The candidate to filter
    ...
    )
{
    ...
    while(TRUE)
    {
        ...
        p->buffer[p->size-1] |= 1;
        p->buffer[0] |= 0x80;
        mpz_bin2mpz(p->buffer, p->size, bnP);
#ifdef  RSA_DEBUG
        if(PrimeSelectWithSieve(bnP, NULL, 0, field, primes))
#else
        if(PrimeSelectWithSieve(bnP, NULL, 0))
#endif
            break;
    }
    MpzTo2B(p, bnP, mpz_num_bytes(bnP));
    mpz_clear(bnP);
    return;
}
...
KDFaContextStart(
		 ...
		 TPM2B           *seed,          // IN: the seed for the digest process
    ...
    )
{
    ...
    for(i = 0; i < UINT32_MAX; i++)
    {
...
#ifdef  RSA_DEBUG
        if(!PrimeSelectWithSieve(bnP, ktxPtr, e, fieldSize, primes))
#else
        if(!PrimeSelectWithSieve(bnP, ktxPtr, e))
#endif
       ...
       mpz_mul(bnN, bnP, bnQ);
       if (mpz_num_bits(bnN) != keySizeInBits)
           FAIL(FATAL_ERROR_INTERNAL);
       // Save the public modulus
       MpzTo2B(n, bnN, n->size);
       // And one prime
       MpzTo2B(p, bnP, p->size);
        ...
        // must exist
        mpz_invert(bnT, bnE, bnT);
        if (mpz_cmp_ui(bnT, 0) == 0)
            FAIL(FATAL_ERROR_INTERNAL);
        ...
        // And, finally, do a trial encryption decryption
        {
            ...
            r.t.buffer[0] &= 0x7F;
                  // Convert
            mpz_bin2mpz(r.t.buffer, r.t.size, bnP);
            mpz_powm(bnQ, bnP, bnE, bnN);
            mpz_powm(bnQ, bnQ, bnT, bnN);
            // If the starting and ending values are not the same, start over )-;
            if(mpz_abscmp(bnP, bnQ) != 0)
            {
                mpz_set_ui(bnQ, 0);
                continue;
            }

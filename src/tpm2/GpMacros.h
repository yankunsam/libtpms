...
/*            $Id: GpMacros.h 847 2016-11-29 21:17:44Z kgoldman $		*/
/*										*/
...
/* 								 Use of TPM_ALG_NULL is reserved for
 								 RSAEP/RSADP testing. If someone is
 								 wanting to test a hash with that
 								 value, don't do it. */
...
/* In some cases, the relationship between two values may be dependent on things that change based
   ...
   sizeof then the preprocessor can't do the comparison. For these cases, we include a special macro
   that, depending on the compiler will generate a warning to indicate if the check always passes or
   always fails because it involves fixed constants. To run these checks, define COMPILER_CHECKS in
   TpmBuildSwitches.h */

...
/*            $Id: Entropy.c 472 2015-12-22 22:43:40Z kgoldman $		*/
/*										*/
...
// consecutive 32-bit values are not the same because (according to FIPS 140-2, annex C
	...
	// "If each call to a RNG produces blocks of n bits (where n > 15), the first n-bit block
	// generated after power-up, initialization, or reset shall not be used, but shall be saved
	// for comparison with the next n-bit block to be generated. Each subsequent generation of
	// an n-bit block shall be compared with the previously generated block. The test shall fail
		  ...
		  )
{
    ...
    /* OK = rand_s(&rndNum) == 0; kgold, gcc doesn't find this */
    rndNum = rand();
    if(OK)
	{
	    if(firstValue)

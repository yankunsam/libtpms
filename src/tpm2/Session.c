...
/*            $Id: Session.c 484 2015-12-31 02:34:18Z kgoldman $		*/
/*										*/
...
// This function is called when the oldest contextID is being loaded or deleted. Once a
// saved context becomes the oldest, it stays the oldest until it is deleted.

// Finding the oldest is a bit tricky.  It is not just the numeric comparison of values
// but is dependent on the value of contextCounter.

// Assume we have a small contextArray with 8, 4-bit values with values 1 and 2 used to
// indicate the loaded context slot number.  Also assume that the array contains hex
// values of (0 0 1 0 3 0 9 F) and that the contextCounter is an 8-bit counter with a
// value of 0x37. Since the low nibble is 7, that means that values above 7 are older
// than values below it and, in this example, 9 is the oldest value.
...
// 8.8.5.3	SequenceNumberForSavedContextIsValid()
...
#define MAX_CONTEXT_GAP ((UINT64)((CONTEXT_SLOT) ~0) + 1)
    ...
    if(     // Handle must be with the range of active sesssions
       handle >= MAX_ACTIVE_SESSIONS
       // the array entry muct be for a saved context
       ||  gr.contextArray[handle] <= MAX_LOADED_SESSIONS
...
// 8.8.5.4	SessionPCRValueIsCurrent()
			...
			)
{
	  ...
	  && s_freeSessionSlots == 1)
	{
	    ...
	    // existing context, then we can't use it because of the abiguity it would
	    // create.
...
TPM_RC
SessionContextLoad(
		   TPM_SESSION_BUFFER  *session,       // IN: session structure from saved context
		   ...
		   )
{
    ...
    MemoryCopy(&s_sessions[slotIndex].session, session,
			  ...
			  )
{
    ...
    MemoryCopy(&bind->t.name[bind->t.size], &auth.t.buffer[overlap],
	       ((UINT32)(auth.t.size - overlap)),
	       ((UINT32)(sizeof(bind->t.name) - bind->t.size)));

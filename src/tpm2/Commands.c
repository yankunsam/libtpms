...
/*            $Id: Commands.c 457 2015-12-08 15:29:45Z kgoldman $		*/
/*										*/
...
IncrementalSelfTest_Out_Marshal(IncrementalSelfTest_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
	...
	UINT32_Marshal(&parameterSize, buffer, size);
    }
    written += TPML_ALG_Marshal(&source->toDoList, buffer, size);
    return written;
}
...
GetTestResult_Out_Marshal(GetTestResult_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_MAX_BUFFER_Marshal(&source->outData, buffer, size);
    written += TPM_RC_Marshal(&source->testResult, buffer, size);
    return written;
}
TPM_RC
...
StartAuthSession_Out_Marshal(StartAuthSession_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMI_SH_AUTH_SESSION_Marshal(&source->sessionHandle, buffer, size);
    if (tag == TPM_ST_SESSIONS) {
	UINT32_Marshal(&parameterSize, buffer, size);
    }
    written += TPM2B_NONCE_Marshal(&source->nonceTPM, buffer, size);
    return written;
}
TPM_RC
...
Create_Out_Marshal(Create_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_PRIVATE_Marshal(&source->outPrivate, buffer, size);
    written += TPM2B_PUBLIC_Marshal(&source->outPublic, buffer, size);
    written += TPM2B_CREATION_DATA_Marshal(&source->creationData, buffer, size);
    written += TPM2B_DIGEST_Marshal(&source->creationHash, buffer, size);
    written += TPMT_TK_CREATION_Marshal(&source->creationTicket, buffer, size);
    return written;
}
TPM_RC
...
Load_Out_Marshal(Load_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM_HANDLE_Marshal(&source->objectHandle, buffer, size);
    if (tag == TPM_ST_SESSIONS) {
	UINT32_Marshal(&parameterSize, buffer, size);
    }
...
ActivateCredential_Out_Marshal(ActivateCredential_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_DIGEST_Marshal(&source->certInfo, buffer, size);
    return written;
}
TPM_RC
...
MakeCredential_Out_Marshal(MakeCredential_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ID_OBJECT_Marshal(&source->credentialBlob, buffer, size);
    written += TPM2B_ENCRYPTED_SECRET_Marshal(&source->secret, buffer, size);
    return written;
}
TPM_RC
...
Unseal_Out_Marshal(Unseal_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_SENSITIVE_DATA_Marshal(&source->outData, buffer, size);
    return written;
}
TPM_RC
...
Duplicate_Out_Marshal(Duplicate_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_DATA_Marshal(&source->encryptionKeyOut, buffer, size);
    written += TPM2B_PRIVATE_Marshal(&source->duplicate, buffer, size);
    written += TPM2B_ENCRYPTED_SECRET_Marshal(&source->outSymSeed, buffer, size);
    return written;
}
TPM_RC
...
Rewrap_Out_Marshal(Rewrap_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_PRIVATE_Marshal(&source->outDuplicate, buffer, size);
    written += TPM2B_ENCRYPTED_SECRET_Marshal(&source->outSymSeed, buffer, size);
    return written;
}
TPM_RC
...
RSA_Encrypt_Out_Marshal(RSA_Encrypt_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_PUBLIC_KEY_RSA_Marshal(&source->outData, buffer, size);
    return written;
}
TPM_RC
...
RSA_Decrypt_Out_Marshal(RSA_Decrypt_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_PUBLIC_KEY_RSA_Marshal(&source->message, buffer, size);
    return written;
}
TPM_RC
...
ECDH_KeyGen_Out_Marshal(ECDH_KeyGen_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ECC_POINT_Marshal(&source->zPoint, buffer, size);
    written += TPM2B_ECC_POINT_Marshal(&source->pubPoint, buffer, size);
    return written;
}
TPM_RC
...
ECDH_ZGen_Out_Marshal(ECDH_ZGen_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ECC_POINT_Marshal(&source->outPoint, buffer, size);
    return written;
}
TPM_RC
...
ECC_Parameters_Out_Marshal(ECC_Parameters_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMS_ALGORITHM_DETAIL_ECC_Marshal(&source->parameters, buffer, size);
    return written;
}
TPM_RC
...
ZGen_2Phase_Out_Marshal(ZGen_2Phase_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ECC_POINT_Marshal(&source->outZ1, buffer, size);
    written += TPM2B_ECC_POINT_Marshal(&source->outZ2, buffer, size);
    return written;
}
TPM_RC
...
EncryptDecrypt_Out_Marshal(EncryptDecrypt_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_MAX_BUFFER_Marshal(&source->outData, buffer, size);
    written += TPM2B_IV_Marshal(&source->ivOut, buffer, size);
    return written;
}
TPM_RC
...
Hash_Out_Marshal(Hash_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_DIGEST_Marshal(&source->outHash, buffer, size);
    written += TPMT_TK_HASHCHECK_Marshal(&source->validation, buffer, size);
    return written;
}
TPM_RC
...
HMAC_Out_Marshal(HMAC_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_DIGEST_Marshal(&source->outHMAC, buffer, size);
    return written;
}
TPM_RC
...
GetRandom_Out_Marshal(GetRandom_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_DIGEST_Marshal(&source->randomBytes, buffer, size);
    return written;
}
TPM_RC
...
HMAC_Start_Out_Marshal(HMAC_Start_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMI_DH_OBJECT_Marshal(&source->sequenceHandle, buffer, size);
    if (tag == TPM_ST_SESSIONS) {
	UINT32_Marshal(&parameterSize, buffer, size);
    }
    return written;
...
SequenceComplete_Out_Marshal(SequenceComplete_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_DIGEST_Marshal(&source->result, buffer, size);
    written += TPMT_TK_HASHCHECK_Marshal(&source->validation, buffer, size);
    return written;
}
TPM_RC
...
EventSequenceComplete_Out_Marshal(EventSequenceComplete_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPML_DIGEST_VALUES_Marshal(&source->results, buffer, size);
    return written;
}
TPM_RC
...
Certify_Out_Marshal(Certify_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ATTEST_Marshal(&source->certifyInfo, buffer, size);
    written += TPMT_SIGNATURE_Marshal(&source->signature, buffer, size);
    return written;
}
TPM_RC
...
Quote_Out_Marshal(Quote_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ATTEST_Marshal(&source->quoted, buffer, size);
    written += TPMT_SIGNATURE_Marshal(&source->signature, buffer, size);
    return written;
}
TPM_RC
...
GetSessionAuditDigest_Out_Marshal(GetSessionAuditDigest_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ATTEST_Marshal(&source->auditInfo, buffer, size);
    written += TPMT_SIGNATURE_Marshal(&source->signature, buffer, size);
    return written;
}
TPM_RC
...
GetTime_Out_Marshal(GetTime_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ATTEST_Marshal(&source->timeInfo, buffer, size);
    written += TPMT_SIGNATURE_Marshal(&source->signature, buffer, size);
    return written;
}
TPM_RC
...
Commit_Out_Marshal(Commit_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ECC_POINT_Marshal(&source->K, buffer, size);
    written += TPM2B_ECC_POINT_Marshal(&source->L, buffer, size);
    written += TPM2B_ECC_POINT_Marshal(&source->E, buffer, size);
    written += UINT16_Marshal(&source->counter, buffer, size);
    return written;
}
TPM_RC
...
EC_Ephemeral_Out_Marshal(EC_Ephemeral_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_ECC_POINT_Marshal(&source->Q, buffer, size);
    written += UINT16_Marshal(&source->counter, buffer, size);
    return written;
}
TPM_RC
...
VerifySignature_Out_Marshal(VerifySignature_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMT_TK_VERIFIED_Marshal(&source->validation, buffer, size);
    return written;
}
TPM_RC
...
PCR_Event_Out_Marshal(PCR_Event_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPML_DIGEST_VALUES_Marshal(&source->digests, buffer, size);
    return written;
}
TPM_RC
...
PCR_Read_Out_Marshal(PCR_Read_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += UINT32_Marshal(&source->pcrUpdateCounter, buffer, size);
    written += TPML_PCR_SELECTION_Marshal(&source->pcrSelectionOut, buffer, size);
    written += TPML_DIGEST_Marshal(&source->pcrValues, buffer, size);
    return written;
}
TPM_RC
...
PCR_Allocate_Out_Marshal(PCR_Allocate_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMI_YES_NO_Marshal(&source->allocationSuccess, buffer, size);
    written += UINT32_Marshal(&source->maxPCR, buffer, size);
    written += UINT32_Marshal(&source->sizeNeeded, buffer, size);
    written += UINT32_Marshal(&source->sizeAvailable, buffer, size);
    return written;
}
TPM_RC
...
PolicySigned_Out_Marshal(PolicySigned_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_TIMEOUT_Marshal(&source->timeout, buffer, size);
    written += TPMT_TK_AUTH_Marshal(&source->policyTicket, buffer, size);
    return written;
}
TPM_RC
...
PolicyGetDigest_Out_Marshal(PolicyGetDigest_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_DIGEST_Marshal(&source->policyDigest, buffer, size);
    return written;
}
TPM_RC
...
ContextSave_Out_Marshal(ContextSave_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMS_CONTEXT_Marshal(&source->context, buffer, size);
    return written;
}
TPM_RC
...
ContextLoad_Out_Marshal(ContextLoad_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMI_DH_CONTEXT_Marshal(&source->loadedHandle, buffer, size);
    if (tag == TPM_ST_SESSIONS) {
	UINT32_Marshal(&parameterSize, buffer, size);
    }
    return written;
...
ReadClock_Out_Marshal(ReadClock_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMS_TIME_INFO_Marshal(&source->currentTime, buffer, size);
    return written;
}
TPM_RC
...
GetCapability_Out_Marshal(GetCapability_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMI_YES_NO_Marshal(&source->moreData, buffer, size);
    written += TPMS_CAPABILITY_DATA_Marshal(&source->capabilityData, buffer, size);
    return written;
}
TPM_RC
...
NV_ReadPublic_Out_Marshal(NV_ReadPublic_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_NV_PUBLIC_Marshal(&source->nvPublic, buffer, size);
    written += TPM2B_NAME_Marshal(&source->nvName, buffer, size);
    return written;
}
TPM_RC
...
NV_Read_Out_Marshal(NV_Read_Out *source, TPMI_ST_COMMAND_TAG tag, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_MAX_NV_BUFFER_Marshal(&source->data, buffer, size);
    return written;
}
TPM_RC

...
/*            $Id: CommandCodeAttributes.c 141 2015-03-16 17:08:34Z kgoldman $	*/
/*										*/
...
// implemented. This function checks to see if the input commandIndex points to an impemented
...
// 9.4.3.2	GetClosestCommandIndex()
...
{
    BOOL                vendor = (commandCode & V_BIT) != 0;
    COMMAND_INDEX       searchIndex = (COMMAND_INDEX)commandCode;
    
    //    int     s_ccAttrSize = sizeof(s_ccAttr)/sizeof(TPMA_CC);
    //    int     s_commandAttributesSize
    //                  = sizeof(s_commandAttributes)/sizeof(COMMAND_ATTRIBUTES);
    //    pAssert(      s_ccAttrSize
    //              == s_commandAttributesSize && s_ccAttrSize == COMMAND_COUNT);
    // if any bit other than the V-bit or a valid command index value is set, then
    // indicate that the command is not implemented.
    if((commandCode & ~V_BIT) != searchIndex)
	return UNIMPLEMENTED_COMMAND_INDEX;
...
#error "Constants are not consistent."
#endif
	    // Check to see if the value is equal to or below the minumum
	    // entry.
	    ...
	    // compiler happy
	    min = LIBRARY_COMMAND_ARRAY_SIZE;       // first vendor cmomand
...
#error  "Something is terribly wrong"
	    ...
	    //            int t = sizeof(s_ccAttr)/sizeof(TPMA_CC);
	    //            int t1 = LIBRARY_COMMAND_ARRAY_SIZE;
	    //            int t2 = VENDOR_COMMAND_ARRAY_SIZE;
			  ...
			  )
{
    ...
    if(!vendor)
	{
	    ...
	    // it look like a large unsigned numbere, this will cause it to fail
...
#else       //%
//% #define GetCommandCode(commandIndex)
//%    ((TPM_CC)(s_ccAttr[commandIndex]commandIndex)
//%        + ((s_ccAttr[commandIndex].V) ? V_BIT : 0))
#endif  //%

// 9.4.3.6	CommandAuthRole()
...
#else   //%
	//% #define EncrptSize(commandIndex)
//%    ((s_commandAttriutes[commandIndex] & ENCRYPT_2) ? 2 : 0)
#endif     //%
...
#else   //%
	//%#define DecryptSize(commandIndex)
//%    ((s_commandAttriutes[commandIndex] & DECRYPT_2) ? 2 : 0)
#endif   //%

// 9.4.3.9	IsSessionAllowed()
...
#else   //%
	//% #define IsSessionAllowed(commandIndex)
//%    ((s_commAttributes[commandIndex] & NO_SESSIONS) == 0)
#endif   //%

// 9.4.3.10	IsHandleInResponse()
...
#else   //%
	//% #define IsHandleInResponse(commandIndex)
//%    ((s_commandAttributes[commandIndex] & R_HANDLE) != 0)
#endif  //%

// 9.4.3.11	IsWriteOperation()
...
#else   //%
	//% #define IsVendorCommand(commandIndex)       ((s_ccAttr[commandIndex].V == SET)
#endif   //%

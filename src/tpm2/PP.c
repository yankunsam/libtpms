...
/*            $Id: PP.c 471 2015-12-22 19:40:24Z kgoldman $			*/
/*										*/
...
// physical presence. The array is an array of bits that has a correspondence with the command code.
				...
				)
{
    ...
    for(commandIndex = 0; commandIndex < COMMAND_COUNT; commandIndex++)
	{
		...
		BitSet(commandIndex, gp.ppList, sizeof(gp.ppList));
	}
    // Write PP list to NV
...
// This function is used to clwar the indicator that a command requires PP confirmation.
			     ...
			     )
{
	...
	BitClear(commandIndex, gp.ppList, sizeof(gp.ppList));
			   ...
			   )
{
    ...
    return (BitIsSet(commandIndex, gp.ppList, sizeof(gp.ppList)));
}
	...
	commandIndex = GetNextCommandIndex(commandIndex))
	{
	    if(PhysicalPresenceIsRequired(commandIndex))
		{
		    if(commandList->count < count)
			{
			    ...
			    // code to it
			    commandList->commandCodes[commandList->count] = GetCommandCode(commandIndex);
			    commandList->count++;
			}
		    else
			{

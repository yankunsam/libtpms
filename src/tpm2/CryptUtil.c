...
/*            $Id: CryptUtil.c 471 2015-12-22 19:40:24Z kgoldman $		*/
/*										*/
...
// should not alter the contents of this buffer until the hash sequence is completed or abandoned.
...
// integer into its canonical form before calling CryptUpdateHash().
...
// This function can be called to complete either an HMAC or hash sequence. The state type
// determines if the context type is a hash or HMAC. If an HMAC, then the call is forwarded to
// CryptCompleteHash().

// If digestSize is smaller than the digest size of hash/HMAC algorithm, the most significant bytes
// of required size will be returned
		  ...
		  )
{
    ...
    if(hashState->type == HASH_STATE_EMPTY)
	return 0;
...
// This function is used to prepare a hash state context for LIB_EXPORT or to import it into the
// internal format. It is used by TPM2_ContextSave() and TPM2_ContextLoad() via
...
{
    EXPORT_HASH_STATE        external;
    
    // Since the _cpri__ImportExportHashState() function doesn't do anything,
    if(IMPORT_STATE == direction)
	MemoryCopy(&external, externalFmt, sizeof(external), sizeof(external));
    _cpri__ImportExportHashState(
				 &internalFmt->state,
				 &external,
				 direction);
    if(EXPORT_STATE == direction)
	MemoryCopy(externalFmt, &external, sizeof(external),
		   sizeof(HASH_STATE_BUFFER));
}
...
// This function generates a key using the KDFa() formulation in Part 1 of the TPM specification. In
// this implementation, this is a macro invocation of _cpri__KDFa() in the hash module of the
// CryptoEngine(). This macro sets once to FALSE so that KDFa() will iterate as many times as
// necessary to generate sizeInBits number of bits.
...
//     This function generates a key using the KDFa() formulation in Part 1 of the TPM
//     specification. In this implementation, this is a macro invocation of _cpri__KDFa() in the
//     hash module of the CryptoEngine(). This macro will call _cpri__KDFa() with once TRUE so that
//     only one iteration is performed, regardless of sizeInBits.
...
// Function to to set the cryptographic elements of an RSA key into a structure to simplify the
// interface to _cpri__ RSA function. This can/should be eliminated by building this structure into
...
// This function provides the interface to _cpri__TestKeyRSA(). If both p and q are provided, n will
// be set to p*q.

// If only p is provided, q is computed by q = n/p. If n mod p != 0, TPM_RC_BINDING is returned.

// The key is validated by checking that a d can be found such that e d mod ((p-1)*(q-1)) = 1. If d
// is found that satisfies this requirement, it will be placed in d.
...
TPM_RC
CryptDecryptRSA(
		UINT16              *dataOutSize,   // OUT: size of plain text in bytes
...
// 10.2.6.17	CryptEcc2PhaseKeyExchange()

// This is the interface to the key exchange function.
...
// 10.2.6.18	CryptIsSchemeAnonymous()
...
// This function creates the sensitive symmetric values for an HMAC or symmetric key. If the
// sensitive area is zero, then the sensitive creation key data is copied. If it is not zero, then
// the TPM will generate a random value of the selected size.
...
// This function is called when the TPM receives a _TPM_Init() indication. After function returns,
// the hash algorithms should be available.
...
// This function does in-place encryption of a buffer using the indicated symmetric algorithm, key,
// IV, and mode. If the symmetric algorithm and mode are not defined, the TPM will fail.
...
#   ifdef  TPM_ALG_CBC
			case TPM_ALG_CBC:
			  ...
			  break;
#   endif
			  // CFB is required and compiler check was done above
			case TPM_ALG_CFB:
...
// This function does in-place decryption of a buffer using the indicated symmetric algorithm, key,
// IV, and mode. If the symmetric algorithm and mode are not defined, the TPM will fail.
...
CryptParameterEncryption(
			 ...
			 TPM2B           *nonceCaller,       // IN: nonce caller
			 UINT16           leadingSizeInByte, // IN: the size of the leading size field in bytes
			 TPM2B_AUTH      *extraKey,          // IN: additional key material other than session auth
...
#   if !defined TPM_ALG_RSAES || !defined TPM_ALG_OAEP
#       error "TPM_ALG_RSAES and TPM_ALG_OAEP are required for RSA"
#   endif
	  case TPM_ALG_RSAES:
	  case TPM_ALG_OAEP:
...
// TPM_RC_SCHEME both scheme and key's default scheme are empty; or scheme is empty while key's
// default scheme requires explicit input scheme (split signing); or non-empty default key scheme
// differs from scheme
...
// the part 2 parser so that the g_implementedAlgorithms vector can be a const. That's not how it is
// now

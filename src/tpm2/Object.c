...
/*            $Id: Object.c 471 2015-12-22 19:40:24Z kgoldman $			*/
/*										*/
...
// processing. It is flushed after command execution. This function is called from ExecuteCommand().
...
// This function loads an object into an internal object structure. If an error is returned, the
		  ...
		  )
{
    ...
    if(ObjectAllocateSlot(handle, &newObject))
	{
	    ...
	    MemoryCopy(newObject, object, sizeof(OBJECT), sizeof(OBJECT));
	}
    return newObject;
}

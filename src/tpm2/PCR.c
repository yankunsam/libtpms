...
/*            $Id: PCR.c 471 2015-12-22 19:40:24Z kgoldman $			*/
/*										*/
...
// Client specification In this implementation, we assume the total number of implemented PCR is 24.

// kgold modified PCR 20-22 locality 3 reset to agree with PC CLient PTP spec.
...
{
    /* kgold modified for PC CLient PTP */
...
#if NUM_POLICY_PCR_GROUP > 0	/* kgold added when no policy groups */
    UINT32           groupIndex;
...
#endif
#if NUM_AUTHVALUE_PCR_GROUP > 0	/* kgold added when no authorization groups */
	  ...
	  )
{
    ...
    if(pcrData != NULL)
	{
	    ...
	    // PCR has changed so uptate the pcrCounter if necessary
	    PCRChanged(handle);
	}
    return;
}
...
void
PCRRead(
	TPML_PCR_SELECTION  *selection,     // IN/OUT: PCR selection, filtered on output
	    ...
	    )
{
    ...
    *sizeNeeded = 0;
    for(i = 0; i < newAllocate.count; i++)
	{
	    ...
	    pcrDrtm = pcrDrtm || BitIsSet(DRTM_PCR,
        ...
        pcrHcrtm = pcrHcrtm || BitIsSet(HCRTM_PCR, 
                                        newAllocate.pcrSelections[i].pcrSelect, 
                                        newAllocate.pcrSelections[i].sizeofSelect);
#else
	    pcrHcrtm = TRUE;
	       ...
	       )
{
    ...
    groupIndex = groupIndex;	/* kgold initialize to eliminate warning when unused */

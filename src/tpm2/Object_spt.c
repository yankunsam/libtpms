...
/*            $Id: Object_spt.c 141 2015-03-16 17:08:34Z kgoldman $		*/
/*										*/
...
// TPM_RC_ATTRIBUTES attempt to inject sensitive data for an asymmetric key; or attempt to create a
// symmetric cipher key that is not a decryption key
...
// TPM_RC_SCHEME inconsistent attributes decrypt, sign, restricted and key's scheme ID; or hash
...
// TPM_RC_TYPE unexpected object type; or non-duplicable storage key and its parent have different
// types
	     ...
	     )
{
    ...
    else if(publicArea->type == TPM_ALG_KEYEDHASH)
	{
		    ...
		    ==  publicArea->objectAttributes.decrypt)
		{
		    ...
		    // If neither sign nor decrypt is SET, then this is a data object. The
		    // TPM does not create data for a data object. So, if the TPM is the
		    // source of the sensitive data, return an error.
		    if(     !publicArea->objectAttributes.sign
			    &&  publicArea->objectAttributes.sensitiveDataOrigin)
			return TPM_RC_ATTRIBUTES;
		}
		...
		{
		    // Here we rely on the unmarshaling code doing the right thing. If
		    // the caller specified TPM_ALG_XOR as the scheme, then the unmarshaling
		    // code would have unmarshaled a TPMS_SCHEME_XOR and the algorithms
		    // would either be one of the allowed values or TPM_ALG_NULL.
		    if(scheme->scheme != TPM_ALG_XOR)
			return TPM_RC_SCHEME;
		    return TPM_RC_SUCCESS;
		    
		}
	    // processing for a signing KEYEDHASH is similar to that of a decryption
	    // key. If the caller specifies TPM_ALG_HMAC, then the unmarshaling code
	    // would have either unmarshaled a valid hash or TPM_ALG_NULL. TPM_ALG_NULL
	    // is valid because it allows the caller to specify the scheme to be used.
	...
	{
	    // Currently, no limits on SYMCIPHER
	}
    else
	return TPM_RC_TYPE;

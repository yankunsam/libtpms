...
/*            $Id: Marshal.c 455 2015-12-03 20:50:12Z kgoldman $		*/
/*										*/
...
{
    UINT16 written = 0;
    written += INT8_Marshal(source, buffer, size);
    return written;
}
...
UINT16
TPM_SU_Marshal(TPM_ST *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += UINT16_Marshal(source, buffer, size);
    return written;
...
UINT16
TPMI_DH_ENTITY_Marshal(TPMI_DH_ENTITY *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += TPM_HANDLE_Marshal(source, buffer, size);
    return written;
...
UINT16
TPMI_RH_HIERARCHY_AUTH_Marshal(TPMI_RH_HIERARCHY_AUTH *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += TPM_HANDLE_Marshal(source, buffer, size);
    return written;
...
UINT16
TPMI_RH_PLATFORM_Marshal(TPMI_RH_PLATFORM *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += TPM_HANDLE_Marshal(source, buffer, size);
    return written;
...
UINT16
TPMI_RH_ENDORSEMENT_Marshal(TPMI_RH_ENDORSEMENT *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += TPM_HANDLE_Marshal(source, buffer, size);
    return written;
...
UINT16
TPMI_RH_PROVISION_Marshal(TPMI_RH_PROVISION *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += TPM_HANDLE_Marshal(source, buffer, size);
    return written;
...
UINT16
TPMI_RH_CLEAR_Marshal(TPMI_RH_CLEAR *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += TPM_HANDLE_Marshal(source, buffer, size);
    return written;
...
UINT16
TPMI_RH_NV_AUTH_Marshal(TPMI_RH_NV_AUTH *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += TPM_HANDLE_Marshal(source, buffer, size);
    return written;
...
UINT16
TPMI_RH_LOCKOUT_Marshal(TPMI_RH_LOCKOUT *source, BYTE **buffer, INT32 *size)
{
    UINT16 written = 0;
    written += TPM_HANDLE_Marshal(source, buffer, size);
    return written;
...
TPMS_AUTH_COMMAND_Marshal(TPMS_AUTH_COMMAND *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_NONCE_Marshal(&source->nonce, buffer, size);
    written += TPMA_SESSION_Marshal(&source->sessionAttributes, buffer, size);
    written += TPM2B_AUTH_Marshal(&source->hmac, buffer, size);
    return written;
}
...
TPMT_SYM_DEF_Marshal(TPMT_SYM_DEF *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMI_ALG_SYM_Marshal(&source->algorithm, buffer, size);
...
TPMS_SENSITIVE_CREATE_Marshal(TPMS_SENSITIVE_CREATE *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPM2B_AUTH_Marshal(&source->userAuth, buffer, size);
    written += TPM2B_SENSITIVE_DATA_Marshal(&source->data, buffer, size);
    return written;
}
...
TPM2B_SENSITIVE_CREATE_Marshal(TPM2B_SENSITIVE_CREATE  *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMS_SENSITIVE_CREATE_Marshal(&source->t.sensitive, buffer, size);
    if (buffer != NULL) {
...
{
    UINT16 written = 0;
    written += TPMI_ALG_SIG_SCHEME_Marshal(&source->scheme, buffer, size);
    written += TPMU_SIG_SCHEME_Marshal(&source->details, buffer, size,source->scheme);
    return written;
}
...
TPMT_RSA_DECRYPT_Marshal(TPMT_RSA_DECRYPT  *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMI_ALG_RSA_DECRYPT_Marshal(&source->scheme, buffer, size);
    written += TPMU_ASYM_SCHEME_Marshal(&source->details, buffer, size, source->scheme);
    return written;
}
...
TPM2B_ECC_POINT_Marshal(TPM2B_ECC_POINT *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMS_ECC_POINT_Marshal(&source->t.point, buffer, size);
    if (buffer != NULL) {
	written += UINT16_Marshal(&written, &sizePtr, size);
    }
    else {
...
#ifdef TPM_ALG_ECDAA
      case TPM_ALG_ECDAA:
	written += TPMS_SIGNATURE_ECDSA_Marshal(&source->ecdaa, buffer, size);
	break;
#endif
#ifdef TPM_ALG_SM2
      case TPM_ALG_SM2:
	written += TPMS_SIGNATURE_ECDSA_Marshal(&source->sm2, buffer, size);
	break;
#endif
#ifdef TPM_ALG_ECSCHNORR
      case TPM_ALG_ECSCHNORR:
	written += TPMS_SIGNATURE_ECDSA_Marshal(&source->ecschnorr, buffer, size);
	break;
#endif
#ifdef TPM_ALG_HMAC
      case TPM_ALG_HMAC:
...
TPM2B_PUBLIC_Marshal(TPM2B_PUBLIC *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMT_PUBLIC_Marshal(&source->t.publicArea, buffer, size);
    if (buffer != NULL) {
        written += UINT16_Marshal(&written, &sizePtr, size);
    }
    else {
...
TPM2B_NV_PUBLIC_Marshal(TPM2B_NV_PUBLIC *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMS_NV_PUBLIC_Marshal(&source->t.nvPublic, buffer, size);
    if (buffer != NULL) {
	written += UINT16_Marshal(&written, &sizePtr, size);
    }
    else {
...
TPM2B_CREATION_DATA_Marshal(TPM2B_CREATION_DATA *source, BYTE **buffer, INT32 *size)
{
    ...
    written += TPMS_CREATION_DATA_Marshal(&source->t.creationData, buffer, size);
    if (buffer != NULL) {
	written += UINT16_Marshal(&written, &sizePtr, size);
    }
    else {

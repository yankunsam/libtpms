...
/*            $Id: Unmarshal.c 455 2015-12-03 20:50:12Z kgoldman $		*/
/*										*/
...
TPM2B_SENSITIVE_Unmarshal(TPM2B_SENSITIVE *target, BYTE **buffer, INT32 *size)
{
	...
	if (rc == TPM_RC_SUCCESS) {
	    rc = TPMT_SENSITIVE_Unmarshal(&target->t.sensitiveArea, buffer, size);
	}
	if (rc == TPM_RC_SUCCESS) {
	    if (target->t.size != startSize - *size) {
		rc = TPM_RC_SIZE;
...
TPM2B_ID_OBJECT_Unmarshal(TPM2B_ID_OBJECT *target, BYTE **buffer, INT32 *size)
{
    ...
    if (rc == TPM_RC_SUCCESS) {
	rc = TPM2B_Unmarshal(&target->b, sizeof(_ID_OBJECT), buffer, size);
    }
    return rc;
}
